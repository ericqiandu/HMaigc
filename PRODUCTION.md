...
涉及 Agent Runtime、Policy、Tool schema、UI protocol、审批或能力契约的发布，必须在同一发布证据集中完成以下检查：

1. **只读审计活动事实。** 由目标 Backend 镜像审计全部活动 Runtime v5 Run、历史非终态 Run、关联 Task、BillingOrder、供应商请求和 Resource；任何未决账务、活动供应商请求、归属冲突或事实损坏都必须阻止切换。
2. **单独批准旧运行退役。** 安全候选只能是已证明尚未跨越供应商、计费、资产写入边界的非终态旧运行。操作者必须核对候选清单并明确批准后，才能在一个事务中写入退役终态与审计事件；审计本身不得修改数据。
3. **验证追加式数据库变更。** 迁移只能新增当前契约所需的表、列和索引，不得覆盖或重写历史运行、账务、资源与事件。目标镜像启动前后都要验证必需唯一索引、外键/作用域约束及重复身份检查。
4. **Backend/Web 同版本切换。** Backend 与 Web 必须来自同一标签、提交和发布清单；禁止先发布一端、让协议不匹配的另一端继续提供服务，也禁止在 Web 或 Backend 保留旧协议兼容分支。
5. **执行上线后协议与幂等烟雾测试。** 至少验证 Runtime v5 / Policy v5 / Tool schema v6 / UI protocol v5、只读能力无审批、写入提案拒绝不改 revision、批准只提交一次、重复批准不重复副作用、SSE 断线补发不重复动作，以及旧 Production/Stage 路由和 UI 不可执行。
6. **验收失败只做镜像回滚。** 任一协议、权限、计费、幂等、事件恢复或历史边界验收失败，立即停止放量并使用已记录的上一 Backend/Web 镜像摘要与同一恢复点回滚；禁止临时打开旧工具、旧执行图、默认模型、兼容路由或运行时 fallback。

上述检查必须记录审计时间、候选与 blocker 数量、操作者、目标版本、索引验证结果、烟雾测试结果和回滚点。没有证据等同于未通过。
