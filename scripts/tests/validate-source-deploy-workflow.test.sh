...
require_text 'id: production-bundle'
require_text 'BUNDLE_ARCHIVE="$RUNNER_TEMP/production-release-bundle.tgz"'
require_text 'BUNDLE_ARCHIVE: ${{ steps.production-bundle.outputs.archive }}'
require_text 'BUNDLE_SHA256: ${{ steps.production-bundle.outputs.sha256 }}'
require_text 'test -s "$BUNDLE_ARCHIVE"'
require_text 'printf '\''%s  %s\n'\'' "$BUNDLE_SHA256" "$BUNDLE_ARCHIVE" | sha256sum -c -'
