...
- 目标版本必须额外通过同源 JS/CSS 发布契约；失败时必须用升级前捕获的当前镜像摘要和恢复点恢复，不重新联网猜测旧版本，也不得用目标策略拒绝恢复旧版本。
...
6. 在当前受版本控制的部署包中运行 `status` 并确认版本、健康接口和 SPA 根入口一致。若旧版本已经满足当前程序资源契约，再运行严格 `verify`；若迁移前旧版本仍使用外部程序资源，必须记录该事实并由下一标签完成硬切换，不能让目标版本策略反向阻断这次升级。
...
- 目标版本的版本、健康接口、SPA 根入口或同源程序资源契约验收失败：自动恢复升级前数据和不可变镜像摘要；恢复只验证旧版本运行事实。
