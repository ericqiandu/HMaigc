...
- 拖拽连线、`@` 引用、Agent `connect_nodes` 与后端 mutation 使用同一方向语义。前端给出可展示的结构化失败原因，后端作为最终权限边界拒绝新建或变更后的非法边。
- 历史非法边不因无关节点更新被阻断；仅当该边本轮被 upsert 时拒绝。
...
4. 扩展 `backend/internal/service/canvas_collaboration_test.go`：新建/更新 Video → Image、Video → Audio 拒绝；Image/Audio → Video 接受；历史非法边加无关节点补丁不失败。
...
- 在 `backend/internal/service/canvas_collaboration_document.go` 对本次 upsert connections 基于合并后节点做类型契约校验。
...
2. 核验数据库：无 schema migration；metadata 新字段向前兼容且无旧逻辑双轨。
