...
describe("membership checkout presentation", () => {
    ...
    test("personal, team, and topup summaries expose only their frozen facts", () => {
        ...
        expect(team).toContain("开通团队版会员");
        expect(team).toContain("开通团队版会员「团队豪华版VIP」");
        expect(team).toContain("团队豪华版VIP");
        expect(team).toContain("membership-team-plan-options");
        expect(team).toContain("membership-team-plan-option is-selected is-locked");
        expect(team).toContain("席位数量");
        expect(team).toContain("membership-team-seat-stepper");
        expect(team).toContain("权益生效");
        expect(team).toContain("支付成功后按现有会员顺延");
        expect(team).toContain("2 席位");
        expect(team).toContain("单席积分");
        expect(team).toContain("32,800");
        expect(team).toContain("团队总积分");
...
describe("checkout implementation boundaries", () => {
    test("the QR and payment CSS consume invariant and semantic color tokens", () => {
        ...
        expect(rootCssProperty(css, ".payment-checkout-shell.is-team", "grid-template-columns")).toBe("minmax(0, 560px) minmax(0, 320px)");
        expect(rootCssProperty(css, ".payment-checkout-shell.is-team", "width")).toBe("min(calc(100% - 48px), 880px)");
        expect(rootCssProperty(css, ".payment-checkout-shell.is-team", "min-height")).toBe("604px");
        expect(cssProperty(css, ".payment-checkout-shell.is-team .payment-checkout-order-surface", "padding")).toBe("0");
        expect(rootCssProperty(css, ".membership-team-plan-options", "grid-template-columns")).toBe("repeat(2, minmax(0, 1fr))");
        expect(cssProperty(css, ".membership-team-plan-option", "min-height")).toBe("92px");
        expect(cssProperty(css, ".membership-team-seat-stepper", "min-height")).toBe("28px");
        expect(cssProperty(css, ".payment-checkout-shell.is-team .payment-checkout-qr-code", "width")).toBe("128px");
        expect(css).toContain("grid-template-columns: minmax(0, 425fr) minmax(0, 341fr)");
        expect(css).toContain("@media (max-width: 767px)");
        expect(cssProperty(css, ".payment-checkout-shell.is-team:not(.is-dialog)", "width")).toBe("min(calc(100% - 32px), 880px)");
