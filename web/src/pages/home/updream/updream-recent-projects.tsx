...
export function UpdreamRecentProjects() {
    ...
    return (
        <section className="updream-recent-projects w-full">
            <div className="updream-recent-projects-inner mx-auto flex w-full flex-col">
                <div className="updream-recent-projects-heading flex items-center justify-between">
                    <h2 className="updream-recent-projects-title">最近项目</h2>
                    <Link className="updream-recent-projects-all inline-flex items-center gap-1 transition-colors" to="/projects">
                        查看全部
                ...
                {projectsQuery.isError ? (
                    ...
                    <div className="updream-recent-projects-grid grid">
                        <Link
                            className="updream-recent-project-create group"
                            to="/projects?create=1"
                        >
                            <span className="updream-recent-project-create-surface flex h-full w-full flex-col items-center justify-center">
                        ...
                        {projectsQuery.isLoading
                            ? Array.from({ length: 2 }, (_, index) => (
                                  <div
                                      key={index}
                                      className="updream-recent-project-skeleton animate-pulse"
                                      aria-label="正在加载最近项目"
                                  />
                              ))
...
function RecentProjectCard({ summary }: { summary: ProjectSummary }) {
    ...
    return (
        <Link
            className="updream-recent-project-card group flex flex-col"
            to={`/projects/${summary.project.id}/overview`}
        >
            <span className="updream-recent-project-preview flex min-h-0 flex-1 items-center justify-center">
                <Clapperboard className="updream-recent-project-preview-icon" />
            </span>
            <span className="updream-recent-project-meta block">
                <strong className="updream-recent-project-name block truncate">
                    {summary.project.name}
                </strong>
                <span className="updream-recent-project-detail flex items-center justify-between gap-2">
