...
import { Copy, Pencil, Plus, Power, RefreshCw, Search, Trash2 } from "lucide-react";
...
export default function StoryboardPromptsPage() {
    ...
    const columns: ColumnsType<StoryboardPromptTemplate> = [
        {
            ...
            width: 190,
            render: (_, template) => (
                <div className="admin-prompt-template-cell">
                    <div className="admin-prompt-template-name font-medium">{template.name}</div>
                    <div className="admin-prompt-template-length text-xs text-foreground/45">{template.content.length} 字符</div>
                </div>
            ),
        },
        { title: "内容摘要", dataIndex: "content", width: 330, render: (content: string) => <div className="admin-prompt-content-summary line-clamp-2 whitespace-pre-wrap text-xs leading-5 text-foreground/65">{content}</div> },
        {
            ...
            width: 96,
            render: (enabled) => (
                <Tag className="admin-prompt-status-tag" variant="filled" color={enabled ? "success" : "default"}>
                    {enabled ? "启用中" : "未启用"}
                </Tag>
            ),
        },
        { title: "更新时间", dataIndex: "updatedAt", width: 154, render: formatTime },
        {
            ...
            render: (_, template) => (
                <AdminRowActions
                    primary={{ label: "编辑", icon: <Pencil className="admin-prompt-edit-icon size-3.5" />, onClick: () => openDrawer(template) }}
                    actions={[
                        {
                            key: "activate",
                            label: "启用版本",
                            icon: <Power className="admin-prompt-activate-icon size-3.5" />,
                            disabled: template.enabled,
                            confirm: { title: "启用这个提示词版本？", description: "启用后会立即替换当前 Agent 分镜生成所使用的版本。", okText: "确认启用" },
                            onClick: () => activate(template),
                        },
                        {
                            key: "delete",
                            label: "删除版本",
                            icon: <Trash2 className="admin-prompt-delete-icon size-3.5" />,
                            danger: true,
                            disabled: template.enabled,
                            confirm: { title: "删除这个提示词版本？", description: "删除后不可恢复；启用中的版本不能删除。", okText: "确认删除" },
    ...
    if (loadError && templates.length === 0) {
        return (
            <AdminPageFrame title="分镜提示词" description="维护 Agent 分镜生成所使用的提示词版本与变量">
                <AdminContentError title="分镜提示词读取失败" description={loadError} onRetry={() => void reload()} />
            </AdminPageFrame>
        );
    }
    ...
    return (
        <AdminPageFrame
            title="分镜提示词"
            description="维护 Agent 分镜生成所使用的提示词版本与变量"
            actions={
        ...
        >
            <AdminDataLayout>
                <AdminMetricBand title="分镜提示词概览" description="启用版本会立即用于 Agent 分镜生成，并与镜头时长、数量和质量约束一起发送给文本模型。">
                    <AdminMetric label="版本总数" value={`${templates.length} 个`} detail="全部历史版本" />
                    <AdminMetric label="当前启用" value={enabledTemplate?.name ?? "未配置"} detail="生产环境生效版本" />
                    <AdminMetric label="未启用版本" value={`${templates.filter((template) => !template.enabled).length} 个`} detail="可编辑或启用" />
                    <AdminMetric label="可用变量" value={`${variables.length} 个`} detail="模板占位符" />
                </AdminMetricBand>
                <AdminFilterSection label="提示词版本筛选">
                    <ListToolbar className="admin-prompt-list-toolbar" active={hasFilters} onReset={() => updateUrl({ filter: "", status: "all" })}>
                        ...
                        <Select
                            className="admin-prompt-status-filter w-32"
                            aria-label="筛选提示词状态"
                            value={status}
                ...
                <AdminContentSection
                    title="提示词版本"
                    description="展开表格行可核对完整内容；启用版本会立即用于 Agent 分镜生成。"
                    actions={
                        <span className="admin-model-billing-content-count">
                            显示 {filtered.length} / {templates.length} 个版本
                        </span>
                    }
                >
                    {loadError ? (
                        <div className="admin-prompt-refresh-error">
                            <AdminContentError title="分镜提示词刷新失败" description={loadError} onRetry={() => void reload()} />
                        </div>
                    ) : null}
                    <TableSurface className="admin-prompt-table-surface">
                        {loading && templates.length === 0 ? (
                            <AdminTableSkeleton rows={8} columns={5} />
                        ) : (
                            <Table
                                className="app-data-table admin-storyboard-prompt-table"
                                size="middle"
                                rowKey="id"
                                loading={loading}
                                columns={columns}
                                dataSource={filtered}
                                locale={{
                                    emptyText: <AdminTableEmpty filtered={hasFilters} title={hasFilters ? "没有匹配的提示词版本" : "还没有提示词版本"} description={hasFilters ? "调整搜索词或状态筛选后重试。" : "创建首个版本并确认内容后，再将其启用。"} />,
                                }}
                                pagination={{ pageSize: 20, showSizeChanger: true, pageSizeOptions: [20, 50, 100], showTotal: (total, range) => `${range[0]}-${range[1]} / 共 ${total} 条` }}
                                scroll={{ x: 892 }}
                                expandable={{ expandedRowRender: (template) => <pre className="admin-expanded-content max-h-80 overflow-auto whitespace-pre-wrap text-xs leading-5">{template.content}</pre> }}
                            />
                        )}
                    </TableSurface>
                </AdminContentSection>
            ...
            <Drawer
                className="admin-object-drawer admin-storyboard-prompt-drawer"
                title={editing ? "编辑分镜提示词" : "新增分镜提示词"}
                open={drawerOpen}
                ...
                destroyOnHidden
                extra={
                    <Button type="primary" loading={saving} disabled={!editorDirty} onClick={() => void save()}>
                        保存
                    </Button>
                }
            >
                <Form className="admin-storyboard-prompt-form" form={form} layout="vertical" requiredMark={false} onValuesChange={() => setEditorDirty(true)}>
                    <Form.Item name="name" label="版本名称" rules={[{ required: true, message: "请填写版本名称" }]}>
                        <Input placeholder="例如：写实电影分镜 v2" />
                    </Form.Item>
                    <Form.Item label="变量">
                        <div className="flex flex-wrap gap-2">
                            {variables.map((variable) => (
                                <Button key={variable.placeholder} size="small" icon={<Copy className="size-3.5" />} onClick={() => insertVariable(variable.placeholder)}>
                                    {variable.label}
                                </Button>
                            ))}
                        </div>
                    </Form.Item>
                    <Form.Item name="content" label="提示词模板" rules={[{ required: true, message: "请填写提示词模板" }]}>
                        <Input.TextArea rows={22} placeholder="使用变量占位符，例如 {{剧情}}、{{用户要求}}、{{画布资产}}" />
                    </Form.Item>
                    <Form.Item name="enabled" label="保存后状态">
                        <Select
                            disabled={editing?.enabled}
                            options={[
                                { label: "保留为未启用版本", value: false },
