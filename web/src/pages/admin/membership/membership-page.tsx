...
export default function MembershipAdminPage() {
    ...
    const [form] = Form.useForm<UpdateMembershipPlanInput>();
    ...
    const load = async () => {
        setLoading(true);
        try {
            const [nextPlans, nextOrders, nextTransactions, nextWebhookEvents] = await Promise.all([
                listAdminMembershipPlans(),
                listAdminMembershipOrders(),
                listAdminPaymentTransactions(),
                listAdminPaymentWebhookEvents(),
    ...
    const openPlan = (plan: MembershipPlan) => {
        setEditing(plan);
        form.setFieldsValue({
            name: plan.name, priceCents: plan.priceCents, originalPriceCents: plan.originalPriceCents,
            creditsPerPeriod: plan.creditsPerPeriod, imageConcurrency: plan.imageConcurrency,
            videoConcurrency: plan.videoConcurrency, topupDiscountBasisPoints: plan.topupDiscountBasisPoints,
            minSeats: plan.minSeats, maxSeats: plan.maxSeats, benefits: plan.benefits,
            enabled: plan.enabled, sortOrder: plan.sortOrder,
        });
    };
    const savePlan = async () => {
        if (!editing) return;
        const values = await form.validateFields();
        await updateAdminMembershipPlan(editing.id, values);
    ...
    return (
        <AdminPageFrame title="会员商业化" description="统一管理个人与团队套餐、并发权益及人工核款订单。">
            <Tabs className="admin-membership-tabs" items={[
                {
                    key: "plans", label: "套餐与权益", children: (
                        <TableSurface className="admin-membership-table-surface mt-0">
                        <Table className="admin-membership-table" rowKey="id" loading={loading} dataSource={plans} pagination={false} size="small"
                            columns={[
                                { title: "套餐", render: (_, row) => <Space><strong>{row.name}</strong><Tag>{row.audience === "team" ? "团队" : "个人"}</Tag><span>{row.billingCycle}</span></Space> },
                                ...
                                { title: "操作", render: (_, row) => <Button type="link" onClick={() => openPlan(row)}>编辑</Button> },
                            ]} />
                        </TableSurface>
                    ),
                },
                {
                    key: "orders", label: "会员订单", children: (
                        <TableSurface className="admin-membership-orders-surface mt-0">
                        <Table className="admin-membership-orders" rowKey="id" loading={loading} dataSource={orders} size="small"
                            columns={[
                                ...
                                { title: "状态", render: (_, row) => <Tag className="admin-membership-order-status" color={row.status === "paid" ? "green" : row.status === "cancelled" ? "default" : row.status === "refunded" ? "blue" : "gold"}>{{ pending: "待支付", paid: "已支付", cancelled: "已关闭", refunded: "已退款" }[row.status]}</Tag> },
                                ...
                                { title: "操作", render: (_, row) => row.status === "pending" ? <Space className="admin-membership-order-actions" size={4}><Button className="admin-membership-confirm-button" type="primary" size="small" onClick={() => setConfirming(row)}>确认收款</Button><Button className="admin-membership-close-button" type="text" danger size="small" onClick={() => setClosing(row)}>关闭</Button></Space> : "—" },
                            ]} />
                        </TableSurface>
                    ),
                },
                {
                    key: "transactions", label: "支付交易", children: (
                        <TableSurface className="admin-payment-transactions-surface mt-0">
                            <Table
                                ...
                                columns={[
                                    ...
                                    { title: "状态", render: (_, row) => <Tag className="admin-payment-status-tag" color={row.status === "paid" ? "green" : row.status === "failed" ? "red" : "gold"}>{row.status}</Tag> },
                ...
                {
                    key: "webhooks", label: "回调审计", children: (
                        <TableSurface className="admin-payment-webhooks-surface mt-0">
                            <Table
                                ...
                                columns={[
                                    ...
                                    { title: "状态", render: (_, row) => <Tag className="admin-webhook-status-tag" color={row.status === "processed" ? "green" : row.status === "rejected" ? "red" : "gold"}>{row.status}</Tag> },
                                    { title: "失败原因", render: (_, row) => row.failureReason || "—", ellipsis: true },
            ...
            <Modal className="admin-membership-plan-modal" title={`编辑 ${editing?.name ?? ""}`} open={Boolean(editing)} onCancel={() => setEditing(null)} onOk={() => void savePlan()} okText="保存">
                <Form className="admin-membership-plan-form" form={form} layout="vertical">
                    <Form.Item className="admin-membership-form-item" name="name" label="名称" rules={[{ required: true }]}><Input /></Form.Item>
                    <Space className="admin-membership-form-row" wrap>
                        <Form.Item name="priceCents" label="售价（分）" rules={[{ required: true }]}><InputNumber min={0} /></Form.Item>
                        <Form.Item name="originalPriceCents" label="原价（分）"><InputNumber min={0} /></Form.Item>
                        <Form.Item name="creditsPerPeriod" label="周期积分（微积分）"><InputNumber min={0} /></Form.Item>
                        <Form.Item name="imageConcurrency" label="图片并发"><InputNumber min={1} /></Form.Item>
                        <Form.Item name="videoConcurrency" label="视频并发"><InputNumber min={1} /></Form.Item>
                        <Form.Item name="topupDiscountBasisPoints" label="充值折扣基点"><InputNumber min={1} max={10000} /></Form.Item>
                        <Form.Item name="minSeats" label="最少席位"><InputNumber min={0} /></Form.Item>
                        <Form.Item name="maxSeats" label="最多席位"><InputNumber min={0} /></Form.Item>
                        <Form.Item name="sortOrder" label="排序"><InputNumber /></Form.Item>
                    </Space>
                    <Form.List name="benefits">{(fields, { add, remove }) => <div className="admin-membership-benefits">{fields.map((field) => <Space className="admin-membership-benefit-row" key={field.key}><Form.Item {...field} rules={[{ required: true }]}><Input placeholder="套餐权益" /></Form.Item><Button onClick={() => remove(field.name)}>移除</Button></Space>)}<Button onClick={() => add("")}>添加权益</Button></div>}</Form.List>
                    <Form.Item className="admin-membership-form-item" name="enabled" label="上架" valuePropName="checked"><Switch /></Form.Item>
                </Form>
            </Modal>
            <Modal className="admin-membership-confirm-modal" title="确认会员订单收款" open={Boolean(confirming)} onCancel={() => setConfirming(null)} onOk={() => void confirmOrder()} okText="确认并开通">
                <Form className="admin-membership-confirm-form" form={confirmForm} layout="vertical">
                    <Form.Item name="providerTradeNo" label="支付流水号" rules={[{ required: true, message: "请输入真实支付流水号" }]}><Input /></Form.Item>
                    <Form.Item name="note" label="核验备注" rules={[{ required: true }]}><Input.TextArea rows={3} /></Form.Item>
                </Form>
            </Modal>
