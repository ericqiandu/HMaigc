...
import { AdminContentSection, AdminDataLayout, AdminFilterSection } from "../components/admin-data-layout";
...
export type UsersPanelApi = {
    listUsers: typeof listAdminUsers;
    bulkDisableUsers: typeof bulkDisableAdminUsers;
    disableUser: typeof deleteAdminUser;
    updateUser: typeof updateAdminUser;
};

const defaultUsersPanelApi: UsersPanelApi = {
    listUsers: listAdminUsers,
    bulkDisableUsers: bulkDisableAdminUsers,
    disableUser: deleteAdminUser,
    updateUser: updateAdminUser,
};

export default function UsersPanel({ onUserChanged, api = defaultUsersPanelApi }: { onUserChanged?: (user: LocalUser) => void; api?: UsersPanelApi }) {
    ...
    }, [api, debouncedFilter, reloadToken, state.page, state.pageSize, state.role, state.status, update]);
    ...
    const bulkDisable = () => {
        modal.confirm({
            ...
            onOk: async () => {
                ...
                try {
                    const result = await api.bulkDisableUsers(selectedUserIds);
    ...
    return (
        <>
            <AdminDataLayout>
                <AdminFilterSection label="用户筛选">
                    <ListToolbar className="admin-users-list-toolbar" active={hasFilters} onReset={resetFilters}>
                        <Input
                            allowClear
                ...
                <AdminContentSection
                    title="用户列表"
                    actions={
                        <div className="admin-users-content-actions flex items-center gap-3">
                            ...
                            >
                                <Button className="admin-users-column-button" icon={<Settings2 className="size-4" />}>
                                    列设置
                                </Button>
                            </Dropdown>
                        </div>
                ...
                >
                    ...
                    <TableSurface className="admin-users-table-surface mt-0">
...
function FilterMenu({ label, value, options, onChange }: { label: string; value: string; options: Array<{ value: string; label: string }>; onChange: (value: string) => void }) {
    ...
    return (
        ...
        >
            <Button className="admin-users-filter-button">
