export type KuaiziMutationScope = "endpoint" | "credential" | `models:${string}`;
...
export function kuaiziAwaitingSyncError(operation: KuaiziAwaitingSyncOperation): Error {
    const scope = operation.scope === "endpoint" ? "服务地址" : operation.scope === "credential" ? "账号凭据" : `${operation.scope.slice("models:".length)} 模型目录`;
