        ...
        >
            ...
            <div className="flex items-start gap-3">
                <motion.span
                    ...
                    transition={status.done || status.error ? { duration: 0.3 } : { duration: 1.5, repeat: Number.POSITIVE_INFINITY }}
                    className="mt-0.5 grid size-9 shrink-0 place-items-center rounded-[14px] border"
                ...
                <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-1 block truncate text-[11px]" style={{ color: theme.node.muted }}>
                        {status.detail}
                    </span>
                </span>
                <span className="shrink-0 rounded-full border px-2 py-1 text-[11px] font-semibold leading-4 tabular-nums" style={{ background: theme.spatial.surface, borderColor: theme.toolbar.border, color: theme.node.muted }}>
...
export function CanvasMergeStatusToast({ progress, theme }: { progress: MergeVideoProgress; theme: CanvasTheme }) {
    ...
    return (
        <div
            data-canvas-no-zoom
            aria-live="polite"
            className="pointer-events-none absolute right-4 top-[164px] z-[90] w-[292px] overflow-hidden rounded-2xl border px-3 py-3 shadow-lg backdrop-blur-xl"
        ...
        >
            <div className="flex items-start gap-2.5">
                ...
                <span className="shrink-0 text-[11px] leading-4 tabular-nums" style={{ color: theme.node.muted }}>
                    {percent}%
                </span>
            </div>
