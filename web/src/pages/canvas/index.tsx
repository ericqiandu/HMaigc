            ...
            <WorkspacePage grid fluid layout="collection" className="canvas-library-page">
                <section className="canvas-library-content min-h-full px-4 pb-8 pt-20 sm:px-6 md:pl-[104px] md:pr-[104px] md:pt-[90px]">
                    <PageHeader
                        title="我的画布"
                        meta={
                            <span className="canvas-library-count text-xs tabular-nums text-foreground/38" aria-live="polite">
                                {hydrated ? filteredProjects.length : "—"}
                            </span>
                        }
                        actions={
                            <>
                                <Button
                                    className="canvas-library-selection-button !h-9 !px-3"
                                    type={selectionMode ? "primary" : "default"}
                                    icon={<CheckSquare2 className="canvas-library-selection-icon size-3.5" />}
                                    onClick={() => setSelectionMode((active) => !active)}
                                >
                                    多选
                                </Button>
                                <Button className="canvas-library-import-button !h-9 !px-3" disabled={!hydrated} icon={<FileUp className="canvas-library-import-icon size-3.5" />} onClick={() => inputRef.current?.click()}>
                                    导入
                                </Button>
                                {projects.length ? (
                                    <Dropdown
                                        overlayClassName="canvas-overlay-dropdown"
                                        menu={{ items: [{ key: "delete-all", danger: true, icon: <Trash2 className="canvas-library-delete-icon size-3.5" />, label: "删除全部画布", onClick: () => setDeleteIds(projects.map((project) => project.id)) }] }}
                                        trigger={["click"]}
                                    >
                    ...
                    >
                        <div className="canvas-library-search min-w-0 w-full sm:w-60">
                            <Input
                                allowClear
                                className="canvas-library-search-input !h-9"
                                prefix={<Search className="canvas-library-search-icon size-3.5 text-foreground/40" />}
                                value={keyword}
                                placeholder="搜索画布"
                                aria-label="搜索画布"
                                onChange={(event) => {
                            ...
                            content={
                                <div className="canvas-library-filter-panel grid w-56 gap-3">
                                    <label className="canvas-library-filter-field grid gap-1.5 text-xs text-foreground/55">
                                        ...
                                        <Select
                                            aria-label="按所属项目筛选"
                                            className="canvas-library-project-filter w-full"
                                            value={projectFilter}
                                            onChange={(value) => {
                                                setProjectFilter(value);
                                                setPage(1);
                                            }}
                                            options={[{ label: "全部项目", value: "all" }, { label: "自由画布", value: "independent" }, ...(projectQuery.data?.projects || []).map(({ project }) => ({ label: project.name, value: project.id }))]}
                                        />
                                    </label>
                                    <label className="canvas-library-filter-field grid gap-1.5 text-xs text-foreground/55">
                                        排序方式
                                        <Select
                                            aria-label="画布排序"
                                            className="canvas-library-sort-filter w-full"
                                            value={sort}
                                            onChange={(value) => {
                                                setSort(value);
                                                setPage(1);
                    ...
                    {selectedIds.length ? (
                        <div className="app-canvas-selection-toolbar mt-3 flex min-h-10 flex-wrap items-center gap-2 rounded-lg bg-foreground/[.05] px-3 py-1.5 text-xs">
                            ...
                            <Button size="small" disabled={!hydrated} icon={<Download className="size-3.5" />} onClick={() => void exportCanvasProjects(selectedProjects, `HMaigc画布-${selectedIds.length}个画布`)}>
                                导出
                            </Button>
                        ...
                        <div className="canvas-library-grid mt-5 grid grid-cols-1 gap-x-3 gap-y-5 sm:grid-cols-[repeat(auto-fill,minmax(240px,260px))] sm:justify-start">
                            {!keyword && projectFilter === "all" ? (
                                <article className="canvas-library-new-card group min-w-0">
                                    <button
                                        type="button"
                                        className="canvas-library-new-button flex aspect-[260/207] w-full flex-col items-center justify-center gap-2 rounded-lg bg-foreground/[.035] text-foreground/55 transition hover:bg-foreground/[.07] hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/30"
                                        disabled={!hydrated}
