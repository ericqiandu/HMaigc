...
export function hasCanvasAgentLaunchRecord(sessions: CanvasAssistantSession[], launchRequestId: string) {
    return sessions.some((session) =>
        session.pendingBackendSession?.launchRequestId === launchRequestId ||
        session.messages.some((message) => objectField(message.detail, "launchRequestId") === launchRequestId),
    );
}

export function hasPendingCinematicAgentWork(sessions: CanvasAssistantSession[]) {
    return sessions.some((session) =>
        session.pendingBackendSession?.kind === "cinematic" ||
        session.messages.some((message) =>
            objectField(message.detail, "kind") === "cinematic-proposal" &&
            objectField(message.detail, "status") === "pending",
        ),
    );
}
...
export function cinematicAgentProgress(detail: AgentSessionDetail): CinematicAgentProgress {
    ...
    const progress = typeof activeTask?.progress === "number"
        ? Math.max(0, Math.min(100, Math.round(activeTask.progress)))
        : undefined;
    ...
    const text = stage
        ? `影视 Agent 正在处理：${stage}${progressText}${taskText}`
        : `影视 Agent 正在处理${progressText}${taskText}`;
