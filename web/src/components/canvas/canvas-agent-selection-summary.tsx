...
export function CanvasAgentSelectionSummary({ config, models, selectedSkills, disabled, readOnly = false, ariaLabel = "本次生成已选配置", onModelsChange, onSkillsChange }: CanvasAgentSelectionSummaryProps) {
    const hasSelections = Boolean(models.image || models.video || selectedSkills.length);
    if (!hasSelections) return null;

    return (
        <div className="canvas-agent-selection-summary" aria-label={ariaLabel}>
            {models.image ? (
                <SelectionChip
                    ...
                    disabled={disabled}
                    onRemove={!readOnly && onModelsChange ? () => onModelsChange({ ...models, image: "" }) : undefined}
                />
            ) : null}
            {models.video ? (
                <SelectionChip
                    ...
                    disabled={disabled}
                    onRemove={!readOnly && onModelsChange ? () => onModelsChange({ ...models, video: "" }) : undefined}
                />
            ) : null}
            {selectedSkills.map((skill) => (
                <SelectionChip
                    ...
                    disabled={disabled}
                    onRemove={!readOnly && onSkillsChange ? () => onSkillsChange(selectedSkills.filter((item) => item.dir !== skill.dir)) : undefined}
                />
            ))}
        </div>
    );
