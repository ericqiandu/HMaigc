...
- 每次 run 同时冻结 `runtimeVersion`、`policyVersion` 与工具 schema 版本，用于恢复、历史审计和未来受控升级。工具 schema v2 已硬切为 `skill.load`、`production.plan`、`production.render`、`canvas.commit` 四个高层工具；旧画布读写和生成提交/等待工具已经从服务端决策 schema 与执行器删除，不保留服务端双轨决策链。v2 启动迁移只会自动退役“工具 schema v1、尚未开始执行且 v1 检查点完整”的排队 run：不存在模型任务/账单时可直接退役；若已经创建模型商业事实，则必须同时满足 Task 已成功且 Billing 已结算，或 Task 已失败/取消且 Billing 已退款。运行记录、`run.failed` 事件和终态 checkpoint 在同一事务写入，失败码固定为 `tool_schema_retired`，保留 v1 工具版本、状态形状以及既有 Task/Billing 事实供审计。活动或不一致的 Task/Billing、已经运行、等待审批、等待工具、含工具事实、未初始化、事实损坏或来自未来 schema 的非终态 run 一律不自动改写；结构完整的剩余不兼容任务会汇总报告全部 ID、状态和版本并拒绝启动，事实损坏则直接报告精确 run 与损坏原因，避免新内核接管既有计费、外部任务或资产所有权。终态历史 run 不阻断升级。
