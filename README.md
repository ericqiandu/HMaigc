...
数据库追加式生产事实、冻结的模型/Skill/工具能力、内部 Specialist 执行与计费、通用 v3 委派端口、阶段审核后端契约、批准后资产发布事务、商业媒体后端执行链、整棵 Run 取消/恢复和迟到结果的 `unadopted` 保全已落地，但它们仍是未从用户入口激活的 v3 迁移基础。阶段审核与资产发布前端卡片、UI protocol v3 与最终硬切尚未完成；当前用户新 Run 仍只走 Runtime v2。硬切后，现有 `agent_production_plan_versions` 与 `agent_production_artifacts` 只保留历史只读用途，不双写、不回填。
...
旧 Runtime 的切换工具当前也只提供显式运维能力：先按 runtime version/status 审计非终态 Run，并同时暴露待审批、待执行/执行中 Tool、内部及旧媒体 Task、活动媒体 Artifact、未结 Billing 与 Checkpoint 问题；只有新表 marker 已安装、Checkpoint 可终止且不存在运行中工具、活动 Task、活动 Artifact 或未结账单时，运维方才可调用安全退役。旧媒体事实通过 `created_by_run_id` 的计划版本继续追踪到 Task 与 Billing，不能因其 operation 或账单幂等键不带 Agent run 前缀而漏审。退役事务追加稳定的 `runtime_schema_retired` 事件、Checkpoint 与失败时间线并中断可终止 Tool；终态历史不改写，重复执行不产生第二份事实。该能力不会自动切换新 Run，当前用户入口仍创建 Runtime v2。
...
- 筷子 GPT Image 2 的后台价格矩阵由模型动态能力目录展开为 `1K/2K/4K × low/medium/high` 九个必配档位。普通画布询价、Agent `production.render` 冻结报价、Task/BillingOrder 预留与结算、供应商调用审计和成本统计都使用同一个 `resolution + quality` 规格身份；任一参数缺失、组合未发布或价格未配置都会在供应商请求前显式失败，不再把画质映射成分辨率，也不使用默认档位。
