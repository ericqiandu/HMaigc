...
- 新建 Run 只可调用工具 schema v5 的 `skill.load`、`specialist.delegate`、`vision.analyze`、`media.generate`、`media.assemble`、`canvas.project`；旧 `production.plan`、`production.render`、`canvas.commit` 只存在于已终结历史事实，不再接受新的模型决策。角色、服装、道具、场景参考图和正式分镜/视频结果只以服务端 Task、BillingOrder、Resource、Artifact Ledger 和交付验收事实为准。

### 单一 Agent Runtime

#### 多专家生产事实（Runtime v4）

数据库追加式生产事实、冻结的模型/Skill/工具能力、内部 Specialist 执行与计费、阶段审核后端契约、批准后资产发布事务、商业媒体与装配执行链、整棵 Run 取消/恢复和迟到结果的 `unadopted` 保全均已接入用户新 Run。当前唯一可执行契约为 Runtime v4 / Policy v4 / Tool schema v5 / UI protocol v4 / Production schema v2；Web 将 `artifact_review`、`stage_review_resolution`、`asset_publication`、`asset_publication_failed` 与 `media_assembly` 的持久 sequence 事件按 `itemId` 合并为可恢复卡片，并只通过精确 Artifact revision 查询正文与候选。用户可逐阶段确认、要求修改或停止；视觉候选必须明确选择，图片或独立音频入库还必须在同一批准请求中明确用途、分类和绑定键。旧 Runtime v3 / Policy v3 / Tool schema v4 的终态历史以及其中的 `assembly_plan.v1` 只读保留，不双写、不回填、不可执行，也不能恢复为可变 Run。
...
- Agent Chat Completions 固定使用供应商 `text/event-stream` 与 `stream_options.include_usage=true`。Run 在等待 worker 时保持 `queued`；worker 真正发起首个供应商请求前先以 CAS 持久化 `queued -> running` 和 `run.status_changed`，SSE 将该事实投影为 `state.snapshot`，因此 Web 在首个可见增量前以可展开真实事件轨迹显示“思考中”，首个增量到达后收起为“已思考”并在下方同一无标题正文持续增长。服务端只从严格 `ModelDecision` JSON 中释放已确认属于顶层 `kind=final` 的 `final.message` 增量；`reasoning_content`、工具参数、澄清结构和交付合同不进入用户正文。每段可见增量与助手 Item 同事务写入连续 sequence，再由 SSE 断线补发；任务断线重领会以新的 `item.started` 原子重置同一消息，协议截断或最终决策无效则追加 `agent.message_failed` 并保留已流出正文。UI 协议 v4 的每个 Item 事件显式携带持久化 `itemKind`，Artifact Item 再以严格 `contentType` 区分审核、决议、发布与装配事实；React 只把 `agent_message` 按 `itemId + sequence` 增量维护为同一个助手气泡，并把同一 Artifact 或装配 `itemId` 的历史与 SSE 重放合并为一张卡。刷新时从零重放耐久事件、仅对新游标执行业务副作用，并只保留有界事件展示窗口；不提供完整响应回退、模型降级或前端假打字。
...
- Web 在启动 Agent run 前先将当前画布未提交变更收敛到权威 revision；首次权限预检只建立远程基线与权限事实，不覆盖 WebSocket 建连前的本地编辑。SSE 只消费协议版本 `4` 的 `AgentUIEvent`，并在收到携带成功 `canvas.project` 工具结果的 `item.completed` 时读取 `committedRevision`，再通过已鉴权的协作查询获取画布事实；查询 revision 低于已确认提交或低于当前本地基线时显式失败或忽略过期响应，禁止旧快照回退已展示的 Agent 交付。
...
- 每次新 run 固定冻结 Runtime v4、Policy v4、工具 schema v5 与 Production schema v2，UI 事件固定为 protocol v4。旧 v3/v3/v4 非终态 Run 不自动接管，服务恢复入口在任何 reconciliation 或写操作前以 `runtime_schema_retired` 拒绝；数据库切换只会在显式审计确认 Tool、Task、Billing、Checkpoint 和 Artifact 均可安全收口后事务化退役。终态 v3 历史继续只读保留原始事实；混合零值、活动状态、商业事实不一致或未来版本记录均拒绝迁移，事件、Checkpoint、Task 与账单事实不改写。
