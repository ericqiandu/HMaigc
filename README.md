...
- Tool schema v6 只接受 `canvas.read`、`canvas.apply_ops`、`assets.read`、`assets.publish`、`media.generate`、`skills.load` 六个原子能力及其严格参数。旧 `skill.load`、Specialist、Production Graph、`vision.analyze`、`media.assemble`、`canvas.project` 等决策不会映射或降级，而是以 `model_decision_invalid` 显式终结。`canvas.read`、`assets.read` 与 `skills.load` 已接入现有画布、资产和 Skill 权威数据源；`canvas.apply_ops` 已接入画布 revision/CAS 真源并由服务端原子执行。每次执行都会重新核验租户、项目、画布、用户权限、基础 revision 与冻结审批提案；输出只保留当前能力契约允许的作用域事实。尚未接入的资产发布和计费能力会显式失败，期间禁止回退旧执行图。
...
当前唯一可创建的新 Run 契约是 Runtime v5 / Policy v5 / Tool schema v6；Web 仍消费 UI protocol v4，UI protocol v5 将在前后端同时切换时单独发布。工具身份固定为 `runId + toolCallId + actionVersion`，已完成工具在重启或重复唤醒后只重放持久结果，绝不再次执行。`canvas.apply_ops`、`assets.publish` 和 `media.generate` 会形成带 SHA-256 的不可变审批提案；审批只对精确提案哈希生效。拒绝、过期或哈希不匹配会写入同一 Run 的失败工具事实并返回运行态继续规划，不伪造批准，也不把 Run 伪装成成功。模型输出不满足严格单 JSON 决策契约时，Run 以 `model_decision_invalid` 明确失败并保留诊断；不会抽取正文、生成默认决策或切换模型。
...
下列 Graph、Stage、Specialist、Artifact Ledger、旧媒体生成和装配结构只用于读取既有 Runtime v4 / Policy v4 / Tool schema v5 / Production schema v2 审计事实，不再接受新模型决策，也不能恢复为活动 Run。相关可执行代码和测试将在硬切清理里程碑删除；在删除前也不得由 Runtime v5 调用。更早的 Runtime v3 / Policy v3 / Tool schema v4 终态历史同样只读保留，不双写、不回填。
...
- Web 在启动 Agent Run 前先将当前画布未提交变更收敛到权威 revision；首次权限预检只建立远程基线与权限事实，不覆盖 WebSocket 建连前的本地编辑。`canvas.apply_ops` 的成功时间线项只发布严格的服务端回执，包括审批提案 SHA-256、基础与已提交 revision、幂等 mutation 身份、已执行 operation 身份及结构化变更证据。Web 不再执行或确认节点/连线写入，只在完整回执通过严格解析后等待已鉴权协作查询读到该 committed revision，再更新远程快照与纯 UI 选区；退役工具、错误画布、畸形回执和过期查询均不会触发客户端副作用。当前后端 SSE 仍投影协议版本 `4`；Web 的 protocol v5 切换与端到端发布属于后续同版本传输里程碑，本中间开发分支不得单独部署。
