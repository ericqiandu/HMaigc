...
11. 用户复核发现 P1：模型选项的底层标签仍拼接渠道名，导致鼠标悬停时出现“丽帧视频”等后台渠道信息。
12. 修复：删除用户端统一标签中的渠道拼接，并同步清理 Agent 文本模型选择器的可见渠道名和悬停标题；渠道 ID 只保留在模型编码值中用于请求路由。
13. 修复后证据：真实浏览器中视频模型选项的 `title` 为 `Seedance 2.0 Fast`、可见文本不含渠道名、页面内“丽帧视频”计数为 `0`；Agent 文本模型选项标题也只包含各自模型显示名称。
...
- 验证画布图片、视频模型以及 Agent 文本模型的选项标题均不拼接供应商或渠道名称。
- 验证非会员专属模型保持不可选。
- 检查浏览器控制台，错误数量为 `0`。
