...
func TestToolSchemaV5ParsesStrictMediaAssemblyDecision(t *testing.T) {
	...
	decision, err := agentruntime.ParseModelDecisionForToolSchema(payload, agentruntime.NextToolSchemaVersion)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := agentruntime.ParseModelDecisionForToolSchema(payload, agentruntime.ProductionToolSchemaVersion); err == nil {
