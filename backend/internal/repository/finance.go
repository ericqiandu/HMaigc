...
func (r *Repository) MarkTokenBillingForReconciliation(id string, providerTaskID string, reason string, next time.Time) error {
	...
	result := r.db.Model(&model.BillingOrder{}).
		...
		Updates(map[string]any{
			"status": model.BillingStatusUncertain, "provider_request_id": providerTaskID, "provider_billing_status": "pending",
...
func (r *Repository) SettleTokenBilling(id string, providerOrderID string, providerAmountSubunits int64, usage TokenUsageFact, now time.Time) error {
	if strings.TrimSpace(id) == "" || strings.TrimSpace(providerOrderID) == "" || providerAmountSubunits < 0 || usage.InputTokens < 0 || usage.CachedTokens < 0 || usage.OutputTokens < 0 || usage.CachedTokens > usage.InputTokens {
	...
	return r.db.Transaction(func(tx *gorm.DB) error {
		...
		if order.Status == model.BillingStatusSettled {
			if order.ProviderBillingOrderID == providerOrderID && order.ProviderBillingAmount == providerAmountSubunits && order.InputTokens == usage.InputTokens && order.CachedTokens == usage.CachedTokens && order.OutputTokens == usage.OutputTokens {
				return nil
			}
			return ErrBillingStateConflict
		}
		...
		if actualAmount > reservedAmount {
			result := tx.Model(&model.BillingOrder{}).Where("id = ? AND status IN ?", order.ID, []model.BillingStatus{model.BillingStatusReserved, model.BillingStatusRunning, model.BillingStatusUncertain}).Updates(map[string]any{
				...
				"provider_billing_status": "amount_exceeds_reservation", "input_tokens": usage.InputTokens, "cached_tokens": usage.CachedTokens,
		...
		if order.TeamID != "" {
			return settleTeamTokenBillingTx(tx, &order, providerOrderID, providerAmountSubunits, actualAmount, reservedAmount, usage, now)
		}
		return settlePersonalTokenBillingTx(tx, &order, providerOrderID, providerAmountSubunits, actualAmount, reservedAmount, usage, now)
	})
}

func settlePersonalTokenBillingTx(tx *gorm.DB, order *model.BillingOrder, providerOrderID string, providerAmountSubunits int64, actualAmount int64, reservedAmount int64, usage TokenUsageFact, now time.Time) error {
	var account model.CreditAccount
	...
	if err := updateSettledTokenOrderTx(tx, order, providerOrderID, providerAmountSubunits, actualAmount, usage, now); err != nil {
		return err
	}
...
func settleTeamTokenBillingTx(tx *gorm.DB, order *model.BillingOrder, providerOrderID string, providerAmountSubunits int64, actualAmount int64, reservedAmount int64, usage TokenUsageFact, now time.Time) error {
	var account model.TeamCreditAccount
...
func updateSettledTokenOrderTx(tx *gorm.DB, order *model.BillingOrder, providerOrderID string, providerAmountSubunits int64, actualAmount int64, usage TokenUsageFact, now time.Time) error {
	result := tx.Model(&model.BillingOrder{}).Where("id = ? AND status IN ?", order.ID, []model.BillingStatus{model.BillingStatusReserved, model.BillingStatusRunning, model.BillingStatusUncertain}).Updates(map[string]any{
		"status": model.BillingStatusSettled, "amount_microcredits": actualAmount, "provider_billing_order_id": providerOrderID,
		"provider_billing_amount": providerAmountSubunits, "provider_billing_status": "succeeded", "input_tokens": usage.InputTokens,
...
func refundBillingOrderTx(tx *gorm.DB, order *model.BillingOrder, errorText string) error {
	if order.TeamID != "" {
		updated := tx.Model(&model.TeamCreditAccount{}).
			Where("team_id = ? AND reserved_microcredits >= ?", order.TeamID, order.AmountMicrocredits).
			Updates(map[string]any{
